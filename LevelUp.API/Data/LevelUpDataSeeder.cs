    ...
    public class LevelUpDataSeeder
    {
        public static async Task SeedAsync(LevelUpDbContext context)
        {
            ...
            if (!await context.Positions.AnyAsync())
            {
                var positions = new List<Position>
                {
                    new Position { Id = Guid.NewGuid(), Title = "Fullstack .NET Developer", IsActive = true },
                    new Position { Id = Guid.NewGuid(), Title = "Fullstack Java Developer", IsActive = true },
                    new Position { Id = Guid.NewGuid(), Title = "Quality Assurance", IsActive = true },
                };
                await context.Positions.AddRangeAsync(positions);
                await context.SaveChangesAsync();
            }
            ...
            {
                var adminAccount = await context.Accounts.FirstAsync(a => a.Role == UserRole.Admin);
                var managerAccount = await context.Accounts.FirstAsync(a => a.Role == UserRole.Manager);
                var employeeAccount = await context.Accounts.FirstAsync(a => a.Role == UserRole.Employee);
            ...
            {
                var managerEmployee = await context.Employees.FirstAsync(e => e.Account.Role == UserRole.Manager);
            ...
            // Seed ModuleItems
            if (!await context.ModuleItems.AnyAsync())
            {
                var modules = await context.Modules.ToListAsync();
                var moduleItems = new List<ModuleItem>();

                // Module 1 items
                var module1 = modules.First(m => m.Title.Contains("ASP.NET Core"));
                moduleItems.AddRange(new[]
                {
                    new ModuleItem { Id = Guid.NewGuid(), ModuleId = module1.Id, Title = "Setup Development Environment", OrderIndex = 1, Descriptions = "Install Visual Studio and .NET SDK", Url = "https://docs.microsoft.com/aspnet/core", IsFinalSubmission = false },
                    new ModuleItem { Id = Guid.NewGuid(), ModuleId = module1.Id, Title = "Create First API", OrderIndex = 2, Descriptions = "Build a simple REST API", Url = "https://docs.microsoft.com/aspnet/core/tutorials", IsFinalSubmission = false },
                    new ModuleItem { Id = Guid.NewGuid(), ModuleId = module1.Id, Title = "Final Project Submission", OrderIndex = 3, Descriptions = "Submit your completed API project", Url = "https://github.com", IsFinalSubmission = true }
                });

                // Module 2 items
                var module2 = modules.First(m => m.Title.Contains("Advanced C#"));
                moduleItems.AddRange(new[]
                {
                    new ModuleItem { Id = Guid.NewGuid(), ModuleId = module2.Id, Title = "Async/Await Deep Dive", OrderIndex = 1, Descriptions = "Understanding async programming patterns", Url = "https://docs.microsoft.com/dotnet/csharp/async", IsFinalSubmission = false },
                    new ModuleItem { Id = Guid.NewGuid(), ModuleId = module2.Id, Title = "LINQ Mastery", OrderIndex = 2, Descriptions = "Master LINQ queries and operators", Url = "https://docs.microsoft.com/dotnet/csharp/linq", IsFinalSubmission = false },
                    new ModuleItem { Id = Guid.NewGuid(), ModuleId = module2.Id, Title = "Delegates and Events", OrderIndex = 3, Descriptions = "Learn delegates, events, and callbacks", Url = "https://docs.microsoft.com/dotnet/csharp/delegates", IsFinalSubmission = false },
                    new ModuleItem { Id = Guid.NewGuid(), ModuleId = module2.Id, Title = "Generics and Collections", OrderIndex = 4, Descriptions = "Deep dive into generic types and collections", Url = "https://docs.microsoft.com/dotnet/csharp/generics", IsFinalSubmission = false },
                    new ModuleItem { Id = Guid.NewGuid(), ModuleId = module2.Id, Title = "Final Project", OrderIndex = 5, Descriptions = "Build a complete async application", Url = "https://github.com", IsFinalSubmission = true }
                });
            ...
            // Additional seeding (Enrollments, EnrollmentItems) bisa diatur serupa
        }
    }
}
